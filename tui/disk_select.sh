...
# _shrink_wizard — Interactive wizard for shrinking a partition to make room for Alpine
...
_shrink_wizard() {
    ...
    dialog_msgbox "No Free Space" \
        "There is not enough free space on ${disk} to create an Alpine partition.\n\n\
...
There is not enough room to free space for Alpine."
        return "${TUI_BACK}"
    fi

    local max_alpine_mib=$(( total_mib - min_part_size ))

    # Default: half available or 20 GiB, whichever is smaller
    local default_alpine=20480
    if [[ $(( max_alpine_mib / 2 )) -lt ${default_alpine} ]]; then
        default_alpine=$(( max_alpine_mib / 2 ))
    fi
    if [[ ${default_alpine} -lt ${ALPINE_MIN_SIZE_MIB} ]]; then
        default_alpine=${ALPINE_MIN_SIZE_MIB}
    fi

    # Step 6: Ask for Alpine size
    local alpine_size_mib
    while true; do
        alpine_size_mib=$(dialog_inputbox "Space for Alpine (MiB)" \
            "How much space to free for Alpine?\n\n\
Partition: ${selected_part} (${selected_fstype})\n\
Total:     ${total_mib} MiB\n\
Used:      ${used_mib} MiB\n\
Maximum:   ${max_alpine_mib} MiB\n\
Minimum:   ${ALPINE_MIN_SIZE_MIB} MiB (8 GiB)\n\n\
Enter size in MiB:" \
            "${default_alpine}") || return "${TUI_BACK}"

        if [[ ! "${alpine_size_mib}" =~ ^[0-9]+$ ]]; then
            dialog_msgbox "Invalid Input" "Please enter a number (MiB)." || true
            continue
        fi

        if [[ ${alpine_size_mib} -lt ${ALPINE_MIN_SIZE_MIB} ]]; then
            dialog_msgbox "Too Small" \
                "Minimum size for Alpine is ${ALPINE_MIN_SIZE_MIB} MiB (8 GiB)." || true
            continue
        fi

        if [[ ${alpine_size_mib} -gt ${max_alpine_mib} ]]; then
            dialog_msgbox "Too Large" \
                "Maximum available space is ${max_alpine_mib} MiB.\n\
(Total ${total_mib} - Used ${used_mib} - 1 GiB safety margin)" || true
    ...
    local new_part_size=$(( total_mib - alpine_size_mib ))
  ...
  Freed space:  ${alpine_size_mib} MiB (for Alpine)\n\n\
Proceed with shrink?" \
        || return "${TUI_BACK}"
...
Type ERASE in the next dialog to confirm."
                    ...
                    if [[ ${free_mib} -lt ${ALPINE_MIN_SIZE_MIB} ]]; then
...
You will then be asked to specify partition paths."
            ...
            PS1="(alpine-partition) \w \$ " bash --norc --noprofile || true
