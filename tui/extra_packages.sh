...
_extra_pkg_state() {
    ...
    case "${pkg}" in
        ...
        niri)              [[ "${ENABLE_NIRI:-no}" == "yes" ]] && echo "on" && return ;;
...
screen_extra_packages() {
    ...
    # --- Hyprland ecosystem ---
    items+=("hyprland-ecosystem" "Hyprland + ecosystem (waybar, wofi, mako...)" "$(_extra_pkg_state hyprland-ecosystem off)")

    # --- niri standalone ---
    items+=("niri" "niri scrollable-tiling Wayland compositor" "$(_extra_pkg_state niri off)")
    ...
    export ENABLE_HYPRLAND ENABLE_NIRI ENABLE_GAMING ENABLE_FINGERPRINT ENABLE_THUNDERBOLT ENABLE_SENSORS ENABLE_WWAN

    # Build EXTRA_PACKAGES from non-special items
    local -a extra_pkgs=()
    for pkg in ${selected}; do
        case "${pkg}" in
            hyprland-ecosystem|niri|gaming|fprintd|bolt|iio-sensor-proxy|ModemManager) ;;
            *) extra_pkgs+=("${pkg}") ;;
        esac
    done

    # Collect any preset EXTRA_PACKAGES not in the checklist above
    local _known_pkgs="fastfetch btop htop kitty vim git tmux v4l-utils hyprland-ecosystem niri gaming fprintd bolt iio-sensor-proxy ModemManager"
    local _preset_extra=""
    local _ep
