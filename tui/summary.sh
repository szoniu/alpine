#!/usr/bin/env bash
# tui/summary.sh — Full summary + confirmation + countdown for Alpine Linux
...
screen_summary() {
    ...
    summary+="\n"
    summary+="\n"
    summary+="Hostname:     ${HOSTNAME:-alpine}\n"
    ...
    case "${DESKTOP_ENV:-kde}" in
        kde)   summary+="Desktop:      KDE Plasma + SDDM + PipeWire\n" ;;
        gnome) summary+="Desktop:      GNOME + GDM + PipeWire\n" ;;
        xfce)  summary+="Desktop:      XFCE + LightDM + PipeWire\n" ;;
        sway)  summary+="Desktop:      Sway + greetd + PipeWire\n" ;;
        niri)  summary+="Desktop:      niri + greetd + PipeWire\n" ;;
        none)  summary+="Desktop:      None (console only)\n" ;;
    esac
    ...
    [[ "${ENABLE_NIRI:-no}" == "yes" ]] && summary+="niri:         standalone enabled\n"
