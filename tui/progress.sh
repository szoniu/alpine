...
readonly -a INSTALL_PHASES=(
    ...
    "bootstrap|Bootstrap Alpine Linux"
...
screen_progress() {
    ...
    dialog_msgbox "Installation Complete" \
        "Alpine Linux has been successfully installed!\n\n\
...
_execute_phase() {
    ...
    case "${phase_name}" in
        preflight)
            ...
            ;;
        disks)
            # Resume onto a disk that already holds a Alpine install but lost
            ...
            if [[ "${MODE:-}" == "resume" ]] && _resume_target_has_system; then
                ...
                ewarn "installed Alpine system but the 'disks' checkpoint is missing."
