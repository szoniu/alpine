    ...
    while true; do
        ...
        case "${choice}" in
            ...
            shell)
                ...
                PS1="(alpine-installer rescue) \w \$ " bash --norc --noprofile || true
                [[ ${_stderr_redirected} -eq 1 ]] && exec 2>>"${LOG_FILE}"
                continue
                ;;
            continue)
...
has_network() {
    ping -c 1 -W 3 alpinelinux.org &>/dev/null || \
...
_scan_partition_for_resume() {
    ...
    if [[ -n "${_RESUME_TEST_DIR:-}" ]]; then
        ...
        if [[ -f "${fake_mp}/tmp/alpine-installer.conf" ]]; then
            _SCAN_HAS_CONFIG=1
        fi
        return 0
    fi
    ...
    if findmnt -rn -S "${part}" &>/dev/null; then
        ...
        if [[ -n "${existing_mp}" ]]; then
            ...
            if [[ -f "${existing_mp}/tmp/alpine-installer.conf" ]]; then
                _SCAN_HAS_CONFIG=1
            fi
            return 0
        fi
    ...
    local mp
    mp=$(mktemp -d "${TMPDIR:-/tmp}/alpine-resume-scan.XXXXXX")
    ...
    if [[ ${mounted} -eq 1 ]]; then
        ...
        if [[ -f "${mp}/tmp/alpine-installer.conf" ]]; then
            _SCAN_HAS_CONFIG=1
        fi
...
_recover_resume_data() {
    ...
    if [[ -n "${_RESUME_TEST_DIR:-}" ]]; then
        ...
        if [[ -f "${fake_mp}/tmp/alpine-installer.conf" ]]; then
            (umask 077; cp "${fake_mp}/tmp/alpine-installer.conf" "${CONFIG_FILE}")
        fi
        return 0
    fi

    local mp
    mp=$(mktemp -d "${TMPDIR:-/tmp}/alpine-resume-recover.XXXXXX")
    local mounted=0
    ...
    if [[ ${mounted} -gt 0 ]]; then
        ...
        if [[ -f "${mp}/tmp/alpine-installer.conf" ]]; then
            (umask 077; cp "${mp}/tmp/alpine-installer.conf" "${CONFIG_FILE}")
...
_infer_from_timezone() {
    ...
    # Alpine: /etc/localtime symlink → zoneinfo
...
_infer_from_keymap() {
    ...
    # Alpine uses console-setup (/etc/default/keyboard)
...
_infer_swap_type() {
    ...
    # zram config (OpenRC service)
    if [[ -f "${mp}/etc/conf.d/zram" ]] || [[ -f "${mp}/etc/init.d/zram-swap" ]]; then
        SWAP_TYPE="zram"
        export SWAP_TYPE
        return 0
    fi
...
infer_config_from_partition() {
    ...
    if [[ -n "${_RESUME_TEST_DIR:-}" ]]; then
        ...
        if [[ -z "${mp}" ]]; then
            mp=$(mktemp -d "${TMPDIR:-/tmp}/alpine-infer.XXXXXX")
            if mount -o ro "${part}" "${mp}" 2>/dev/null; then
                need_unmount=1
            elif [[ "${fstype}" == "btrfs" ]]; then
                if mount -o ro,subvol=@ "${part}" "${mp}" 2>/dev/null; then
...
# an installed Alpine system. A missing 'disks' checkpoint does NOT mean the
...
_resume_target_has_system() {
    ...
    for opt in "ro,subvol=@" "ro"; do
        if mount -o "${opt}" "${root}" "${probe}" 2>/dev/null; then
            # Alpine markers: the release file, the apk database, or an
            # ID=alpine os-release. Any one is enough — a half-finished
            # bootstrap still deserves protection from a reformat.
            if [[ -f "${probe}/etc/alpine-release" ]] || \
               [[ -d "${probe}/lib/apk/db" ]] || \
               grep -q '^ID=alpine' "${probe}/etc/os-release" 2>/dev/null; then
                found=0
            fi
