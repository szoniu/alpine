#!/usr/bin/env bash
# constants.sh — Global constants for the Alpine Linux installer
source "${LIB_DIR}/protection.sh"

readonly INSTALLER_VERSION="1.0.0"
readonly INSTALLER_NAME="Alpine Linux TUI Installer"

# Paths (use defaults, allow override from environment)
: "${MOUNTPOINT:=/media/root}"
: "${CHROOT_INSTALLER_DIR:=/tmp/alpine-installer}"
: "${LOG_FILE:=/tmp/alpine-installer.log}"
: "${CHECKPOINT_DIR:=/tmp/alpine-installer-checkpoints}"
: "${CHECKPOINT_DIR_SUFFIX:=/tmp/alpine-installer-checkpoints}"
: "${CONFIG_FILE:=/tmp/alpine-installer.conf}"

# Partition sizes (MiB)
readonly ESP_SIZE_MIB=512
readonly SWAP_DEFAULT_SIZE_MIB=4096
: "${ALPINE_MIN_SIZE_MIB:=4096}"

# Gum TUI backend
: "${GUM_VERSION:=0.17.0}"
: "${GUM_CACHE_DIR:=/tmp/alpine-installer-gum}"
