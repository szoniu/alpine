...
log_relocate_to_target() {
    ...
    local target="${target_dir}/alpine-installer.log"
    ...
    LOG_FILE="${target}"
    SKIPPED_LOG="${target_dir}/alpine-installer-skipped.log"
...
skipped_step_record() {
    local desc="$1"
    : "${SKIPPED_LOG:=/tmp/alpine-installer-skipped.log}"
...
skipped_steps_report() {
    local f="${SKIPPED_LOG:-/tmp/alpine-installer-skipped.log}"
    [[ -s "${f}" ]] || return 0
