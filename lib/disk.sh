...
# Uses sfdisk (util-linux) for atomic GPT partitioning — Alpine Linux edition
    ...
    case "${fs}" in
        ext4)
            disk_plan_add "Format root as ext4" \
                mkfs.ext4 -L alpine "${ROOT_PARTITION}"
            ;;
        btrfs)
            disk_plan_add "Format root as btrfs" \
                mkfs.btrfs -f -L alpine "${ROOT_PARTITION}"
            ;;
        xfs)
            disk_plan_add "Format root as XFS" \
                mkfs.xfs -f -L alpine "${ROOT_PARTITION}"
            ;;
    esac
    ...
    case "${fstype}" in
        ...
        btrfs)
            disk_plan_add "Shrink btrfs filesystem on ${part}" \
                bash -c "mkdir -p /mnt/alpine-shrink-tmp && mount ${part} /mnt/alpine-shrink-tmp && btrfs filesystem resize ${new_size}M /mnt/alpine-shrink-tmp; rc=\$?; umount /mnt/alpine-shrink-tmp 2>/dev/null; rmdir /mnt/alpine-shrink-tmp 2>/dev/null; exit \$rc"
            ;;
    esac
